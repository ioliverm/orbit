...
- **Editable past vesting events** (v1.4): new "Precios de vesting" section on grant detail lists vest events with `vest_date ≤ today`. Each row is inline-editable for `vest_date`, `shares_vested_this_event`, and `fmv_at_vest`; FMV is also editable on future rows. Date + shares edits are confined to past events only. Edits mark the row `is_user_override = true`; grant re-derivations preserve overridden rows. Cumulative invariant (sum = share_count) is RELAXED on grants with any override — user-edit wins. Bulk-fill CTA "Aplicar FMV a todos" for pre-IPO grants with a constant 409A (SKIPS rows that already carry a manual FMV — safer mental model). Warning banner surfaces on the grant-edit form if any overrides exist.
- **Paper-gains current-price entry** (v1.4 follow-up to Q1 2026-04-21): one current-price input per **ticker** in the user's portfolio, with a per-grant override affordance on grant-detail. Grants without a ticker (unlisted-company ESPP/NSO) fall back to a per-grant field.
- **Paper-gains tile partial-data UX** (v1.4 follow-up to Q3 2026-04-21): when some past vests have NULL `fmv_at_vest`, the tile renders gains for grants with complete FMV and surfaces a "complete your data" banner listing the grants needing data — it does NOT fall back to grant-date-strike as a basis proxy.
...
- **No `sessions.country_iso2` GeoIP population** (Q2 2026-04-21 decision: defer to Slice 9 alongside Finnhub + other vendor procurement). ADR-016's "Slice 3 populates" note is superseded; column stays NULL through Slice 8.
