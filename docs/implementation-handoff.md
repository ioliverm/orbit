...
```
Slice 0a (local foundations) ──▶ Slice 1 ──▶ Slice 2 ──▶ … ──▶ Slice 7 ──▶ Slice 8 (bulk import) ──▶ Slice 9 (production deploy + launch)
```

Each slice is independently shippable and demo-able on `localhost` through Slice 8. Do not start Slice 1 until every Slice-0a checklist item is green. Slice 9 is entered only once Slice 8 is green — and no external user touches the system before Slice 9 closes.

**Local-first-until-launch (decision 2026-04-19):** cloud deployment was deferred to a single concentrated final slice. Slices 0a–8 run against Docker Compose Postgres + Vite dev server on a developer machine. Every ADR-015 §0b item, plus the pen-test and legal-surface publication previously scoped to Slice 7, consolidates into **Slice 9 — Production deployment & launch gate** in `requirements/v1-slice-plan.md`.

**Bulk-import-deferred (decision 2026-04-20, v1.3):** CSV (Carta + Shareworks) and ETrade PDF import were moved out of Slice 2 and into a new **Slice 8 — Portfolio bulk import**, between Slice 7 and the deploy slice (now Slice 9). Rationale: hand-entry through Slice 7 is Ivan's own dogfooding path; import tooling has a long-tail QA story and is safer to build in one batch right before production users arrive. Slice 2 shrinks to M (ESPP purchases, Art. 7.p trips, multi-grant dashboard, Modelo 720 category inputs, session-device UI).
...
- **Slice 2 and beyond**: sequenced in [`requirements/v1-slice-plan.md`](requirements/v1-slice-plan.md) v1.3. Do not speculate; come back when the prior slice is green.
- **Slice 9 starting point** (when Slice 8 is green): the ADR-015 §0b item set + deferred commercial/legal cutovers catalogued in the Slice 9 section of `requirements/v1-slice-plan.md`.
